...
#define MAX_EVENT_HANDLERS_PER_EVENT 200
		...
		script_s* LoadScriptFile(const char* filename)
		{
			...
			fp = FileSystem::FileOpen(pathname, "rb");
			length = FileSystem::FileSize(fp);
			if (!fp) return 0;
			...
			memcpy(script->buffer, FileSystem::ReadBytes(fp, length).data(), length);
		...
		operationEnum Expression_GetOp(const char* token)
		{
			...
			{
				if (!stricmp(token, g_expOperatorNames[opNum]))
				{
		...
		std::string __cdecl GetOperandAsString(Operand operand)
		{
			...
			{
			case VAL_STRING:
				return va("%s", operand.internals.stringVal.string);
			case VAL_INT:
				...
				if (op == OP_NOOP)
				{
					...
					case TT_NUMBER:
						if (token.floatvalue == (float)(token.intvalue))
						{
							...
							newExpression->data.op = OP_NOOP;
							newExpression->data.operand.internals.intVal = token.intvalue;
						}
						else
						{
							newExpression->type = 1;
							newExpression->data.op = OP_RIGHTPAREN;
					...
					case TT_NAME:
						newExpression->type = 1;
						newExpression->data.op = OP_MULTIPLY;
		...
		int PC_Char_Parse(/*int handle,*/ char* out)
		{
			pc_token_s token;
		...
		int __cdecl PC_Script_Parse(/*int handle,*/ const char** out) // not tested
		{
			char dst[0x1400];
			pc_token_s pc_token;
		...
		int __cdecl PC_EventScript_Parse(/*int handle,*/ MenuEventHandlerSet** eventHandlerSet)
		{
			pc_token_s token;
			...
			*eventHandlerSet = zmem->Alloc<MenuEventHandlerSet>();
			...
			while (1)
			{
				...
				if (!stricmp(token.string, "if"))
					eventType = EVENT_IF;
				else if (!stricmp(token.string, "else"))
					eventType = EVENT_ELSE;
				else if (!stricmp(token.string, "setLocalVarBool"))
					eventType = EVENT_SET_LOCAL_VAR_BOOL;
				else if (!stricmp(token.string, "setLocalVarInt"))
					eventType = EVENT_SET_LOCAL_VAR_INT;
				else if (!stricmp(token.string, "setLocalVarFloat"))
					eventType = EVENT_SET_LOCAL_VAR_FLOAT;
				else if (!stricmp(token.string, "setLocalVarString"))
				...
				else
				{
					...
					do
					{
						if (!PC_ReadTokenHandle(/*handle,*/ &token))
							return 0;
						if (!stricmp(token.string, "}"))
						{
							PC_UnreadLastTokenHandle(/*handle*/);
							break;
						}
						else if (!stricmp(token.string, "{"))
						{
		...
		int MenuParse_visible(menuDef_t* menu/*, int handle*/)
		{
			...
			if (!PC_ReadTokenHandle(/*handle,*/ &token))
				return 0;
			if (!stricmp(token.string, "when") || !stricmp(token.string, "if"))
			{
		...
		int MenuParse_background(menuDef_t* menu/*, int handle*/)
		{
			...
			strlwr((char*)menu->window.background->name);
			return 1;
		}
		...
		int MenuParse_execExp(menuDef_t* menu/*, int handle*/)
		{
			...
			if (!stricmp(expressionType, "visible"))
			{
				if (!PC_ReadTokenHandle(/*handle,*/ &token))
				{
			...
			else if (!stricmp(expressionType, "rect"))
			{
				if (!PC_ReadTokenHandle(/*handle,*/ &token))
				{
			...
			else if (!stricmp(expressionType, "openSound"))
			{
				if (Expression_Read(/*handle,*/ &menu->data->openSoundExp))
					return 1;
			}
			else if (!stricmp(expressionType, "closeSound"))
			{
				if (Expression_Read(/*handle,*/ &menu->data->closeSoundExp))
					return 1;
			}
			else if (!stricmp(expressionType, "soundLoop"))
			{
		...
		{
			if (!PC_Float_Parse(/*handle,*/ &menu->data->blurWorld))
				return 0;
			if (menu->data->blurWorld >= 0.0)
				return 1;
			PC_SourceError(/*handle,*/ "blur must be >= 0; %g is invalid", menu->data->blurWorld);
			return 0;
		}
		...
		char* UI_FileText(const char* fileName)
		{
			...
			len = FileSystem::FileSize(fp);
			if (!fp)
				return 0;
			if (len < sizeof(buffer))
			{
				memcpy(buffer, FileSystem::ReadBytes(fp, len).data(), len);
				buffer[len] = 0;
				FileSystem::FileClose(fp);
				result = buffer;
			}
			...
			{
				FileSystem::FileClose(fp);
				ZONETOOL_ERROR("Menu file %s is larger than the %i byte buffer used to parse menu files", fileName, sizeof(buffer));
				result = 0;
			}
			return result;
		}
		...
		int ItemParse_background(itemDef_t* item/*, int handle*/)
		{
			...
			strlwr((char*)item->window.background->name);
			return 1;
		}
		...
		int ItemParse_execExp(itemDef_t* item/*, int handle*/)
		{
			...
			else if (!stricmp(expressionType, "text"))
			{
				if (item->textExp)
				{
					PC_SourceError(/*handle,*/ "redefinition of expression for '%s'.", expressionType);
			...
			else if (!stricmp(expressionType, "material"))
			{
				if (item->materialExp)
				{
					PC_SourceError(/*handle,*/ "redefinition of expression for '%s'.", expressionType);
			...
			else if (!stricmp(expressionType, "disabled"))
			{
				if (item->disabledExp)
				{
					PC_SourceError(/*handle,*/ "redefinition of expression for '%s'.", expressionType);
			...
			{
				if (!stricmp(expressionType, g_itemFloatExpressions[i].s1) && !stricmp(expressionComponent, g_itemFloatExpressions[i].s2))
				{
		...
		int ItemParse_selectIcon(itemDef_t* item/*, int handle*/)
		{
			...
			strlwr((char*)listPtr->selectIcon->name);
			return 1;
		}
		...
		parse_itemdef_func* find_itemdef_func(const char* keyword)
		{
			...
			{
				if (!stricmp(keyword, p_id_funcs.at(i).keyword))
				{
		...
		parse_menudef_func* find_menudef_func(const char* keyword)
		{
			...
			{
				if (!stricmp(keyword, p_md_funcs.at(i).keyword))
				{
		...
		bool UI_ParseMenuInternal(const char* menuFile)
		{
			...
			if (handle)
			{
				while (PC_ReadTokenHandle(/*handle,*/ &token))
				{
					if (stricmp(token.string, "}") && stricmp(token.string, "{"))
					{
						if (stricmp(token.string, "assetGlobalDef"))
						{
							if (stricmp(token.string, "menudef"))
							{
								PC_SourceError(
									/*handle,*/
		...
		void IMenuList::emit_menu_def(menuDef_t* asset)
		{
			...
			if (asset->data)
			{
				...
				emit_float("fadeCycle", asset->data->fadeCycle);
				...
				emit_float("blurWorld", asset->data->blurWorld);
		...
		void IMenuList::dump(MenuList* asset)
		{
			...
			fp = FileSystem::FileOpen(path, "wb");
			if (fp)
			{
